...
def main() -> int:
    ...
    window.surface.show_view("all")
    pump(app_global, 4)
    save(window, out / "05-all.png")

    window.float_window._expanded = True
    window.float_window._apply_size()
    window.float_window.sync()
    pump(app_global, 4)
    save(window.float_window, out / "06-float-expanded.png", desktop=True)

    window.float_window._expanded = False
    window.float_window._apply_size()
    window.float_window.sync()
    pump(app_global, 3)
    save(window.float_window, out / "07-float-pill.png", desktop=True)

    from ui.settings_dialog import SettingsDialog
    dialog = SettingsDialog(store, window)
    save(dialog, out / "08-settings.png")
