...
class TaskRow(QFrame):
    ...
    def __init__(self, task: Task, now: datetime, active_task_id: str | None, parent=None,
                 compact: bool = False, default_min: int = 25):
        super().__init__(parent)
        self.task = task
...
class TodayView(QWidget):
    ...
    def _buckets(self, now: datetime) -> list[tuple[str, list[Task]]]:
        pool = self.store.active_tasks()
        start_of_day = now.replace(hour=0, minute=0, second=0, microsecond=0)
        week_end = start_of_day + timedelta(days=8)
        return [
            ("已逾期", [t for t in pool if not t.done and t.is_overdue(now)]),
            ("今天要做", [t for t in pool if not t.done and t.due_dt
                       and start_of_day <= t.due_dt < start_of_day + timedelta(days=1) and not t.is_overdue(now)]),
            ("接下来一周", [t for t in pool if not t.done and t.due_dt
                       and start_of_day + timedelta(days=1) <= t.due_dt < week_end]),
            ("待安排", [t for t in pool if not t.done and t.due_dt is None]),
            ("今天已完成", [t for t in pool if t.done and t.done_at and _safe_date(t.done_at) == now.date()]),
        ]
    ...
    def refresh(self, now: datetime, active_task_id: str | None) -> None:
        ...
        key = [[(name, tuple((t.id, t.focus_min) for t in tasks)) for name, tasks in buckets],
        ...
        accents = {"已逾期": theme.DANGER, "今天要做": theme.ACCENT, "今天已完成": theme.DONE}
